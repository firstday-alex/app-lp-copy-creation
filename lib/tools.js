      ...
      items:{ type:'object', additionalProperties:false, properties:{
        ...
        variants:{ type:'array', description:'Option 1 stays closest to the current copy and the current length; each later option is a bigger swing. Never more than the requested number.',
          items:{ type:'object', additionalProperties:false, properties:{
            ...
            word_count:{type:'integer', description:'your own count of the words in text, so an out-of-range line is visible without re-reading it'},
            ...
            verify:{ type:'array', description:'one entry per fact you needed and did not have; each becomes an inline [VERIFY: …] in the line itself',
              items:{type:'string'} },
            copy_checks:{ type:'array', description:'this variant against each Copy_Checks rule',
              items:{ type:'object', additionalProperties:false, properties:{
                id:{type:'string'}, score:{type:'integer', enum:[0,1,2]}, justification:{type:'string'} },
                required:['id','score'] } },
